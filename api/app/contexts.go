...
// OK sends a HTTP response with status code 200.
func (ctx *ListRecipeContext) OK(r RecipeRecipeCollection) error {
	if ctx.ResponseData.Header().Get("Content-Type") == "" {
		ctx.ResponseData.Header().Set("Content-Type", "application/recipe.recipe+json; type=collection")
	}
	if r == nil {
		r = RecipeRecipeCollection{}
	}
	return ctx.ResponseData.Service.Send(ctx.Context, 200, r)
}

// OKIngredient sends a HTTP response with status code 200.
func (ctx *ListRecipeContext) OKIngredient(r RecipeRecipeIngredientCollection) error {
	if ctx.ResponseData.Header().Get("Content-Type") == "" {
		ctx.ResponseData.Header().Set("Content-Type", "application/recipe.recipe+json; type=collection")
	}
	if r == nil {
		r = RecipeRecipeIngredientCollection{}
	}
	return ctx.ResponseData.Service.Send(ctx.Context, 200, r)
}
